...
export const Moves: import("../../../sim/dex-moves").ModdedMoveDataTable = {
  ...
  inverseroom: {
    ...
    condition: {
      ...
      onFieldStart(target, source) {
        ...
        this.add(
          "-message",
          "It created a bizarre area in which all type matchups are reversed!",
        );
      },
      ...
      onFieldEnd() {
        ...
        this.add(
          "-message",
          "Inverse Room wore off, and all type matchups returned to normal!",
        );
      },
  ...
  actualize: {
    ...
    onHit(target, source) {
      ...
      if (sourceTypes.length > 1) {
        if (
          sourceTypes[1] === targetTypes[1] ||
          targetTypes[1] === sourceTypes[0]
        ) {
          return;
        }
  ...
  excalibur: {
    ...
    type: "Fairy",
    contestType: "Beautiful",
    shortDesc: "Breaks screens, lowers the user's Defense and Speed by 1.",
    contributors: ["Rudy H."],
  },
  magicmissile: {
    num: 1011,
  ...
  petrify: {
    ...
    onTryImmunity(target) {
      return !target.hasType("Rock");
    },
    target: "normal",
    type: "Rock",
    shortDesc: "Paralyzes and Salt Cures the target.",
    contestType: "Beautiful",
    contributors: ["Jared G."],
  },
  corkscrew: {
    ...
    target: "normal",
    type: "Fighting",
    shortDesc: "Ends user and target hazards.",
    contestType: "Cool",
    contributors: ["Jared G."],
  },
  kaleidoscope: {
  ...
  maskslip: {
    ...
    flags: { contact: 1, protect: 1, mirror: 1, metronome: 1 },
    onHit(target, source) {
      const oldAbility = source.setAbility(
        this.sample(["magician", "prankster"]), // idk if this should check for if the current ability is actually prankster/magician
      );
  ...
  rockthrow: {
    ...
    shortDesc:
      "Transfers Stealth Rock from the user's to the foe's side if the foe does not have it.",
    contributors: ["Bryce G."],
  },
  rocksmash: {
    inherit: true,
  ...
  mist: {
    // Also implementing the logic for Ancient Sundial
    inherit: true,
    modified: "DigiPen",
    condition: {
      inherit: true,
      ...
      onTryBoost(boost, target, source, effect) {
        ...
        if (
          ...
          (effect.effectType === "Move" && effect.category === "Status")
        ) {
          let showMsg = false;
          let i: BoostID;
          for (i in boost) {
  ...
  luckychant: {
    // Also implementing the logic for Ancient Sundial; not sure how to do alternate abilities effect
    inherit: true,
    modified: "DigiPen",
    condition: {
      inherit: true,
  ...
  gravity: {
    // Implementing the logic for Ancient Sundial
    inherit: true,
    condition: {
      inherit: true,
      durationCallback(source, effect) {
  ...
  magicroom: {
    // Implementing the logic for Blueprint
    inherit: true,
    condition: {
      inherit: true,
      durationCallback(source, effect) {
